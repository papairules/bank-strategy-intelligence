...
const jobDetail = {
  job: { ...record, title: record.job_title, description: "Analytics role.", source_job_id: "R-1", country: "India", capability_classifications: [], posted_date: "2026-08-20", closing_date: null, employment_type: "full_time", seniority_level: null, is_leadership: false },
  evidence: { evidence_id: record.evidence_id, source_url: record.source_url, source_type: record.source_type, source_title: record.job_title, retrieved_at: record.captured_at, source_excerpt: record.evidence_preview, raw_reference: "workday:R-1", collector_identity: "collector", provenance_metadata: {} },
  enrichment: null,
};
...
describe("EvidencePage", () => {
  it("renders summary, records, enriched state, and detail relationships", async () => {
    ...
    expect(screen.getByText("No deterministic technology signals currently cite this evidence.")).toBeInTheDocument();
  });
  ...
  it("opens related job intelligence from an evidence record", async () => {
    vi.stubGlobal("fetch", vi.fn((input: RequestInfo | URL) => {
      const url = String(input);
      if (url.includes("/summary")) return ok(summary);
      if (url.includes("/hiring/jobs/")) return ok(jobDetail);
      return ok({ items: [record], total: 1, limit: 25, offset: 0, returned_count: 1 });
    }));
    const user = userEvent.setup();
    const view = render(withOrg(<EvidencePage />));
    await user.click(await view.findByRole("button", { name: "Senior Analytics Consultant" }));
    expect(await view.findByRole("dialog", { name: "Job detail" })).toBeInTheDocument();
    expect(view.getByText("Analytics role.")).toBeInTheDocument();
  });
