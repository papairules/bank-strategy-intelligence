...
export function TechnologyIntelligencePage() {
  ...
  const sourceTechnologyJobs = summary.data?.snapshot.source_technology_jobs ?? summary.data?.snapshot.enriched_jobs ?? 0;
  const sourceTechnologyObservations = summary.data?.snapshot.source_technology_observation_count ?? summary.data?.snapshot.technology_observation_count ?? 0;

  return (
    <div className="page">
      <div className="page-heading">
        <div><span className="eyebrow">Technology Intelligence</span><h1>Observed technology demand from hiring evidence</h1><p>Traceable technologies extracted from authoritative source text and persisted enrichment. Observations indicate hiring evidence—not confirmed organization-wide technology strategy.</p></div>
      </div>

      {summary.loading && <LoadingState label="Loading technology coverage…" />}
      {summary.error && <ErrorState message={summary.error} />}
      {summary.data && <>
        <div className="technology-coverage-notice"><strong>Technology support coverage</strong><span>Technology classifications are available for {summary.data.snapshot.enriched_jobs} of {summary.data.snapshot.total_jobs} observed hiring records.</span><span>Deterministic source matches cover {sourceTechnologyJobs} jobs; AI enrichment covers {summary.data.snapshot.enriched_jobs}.</span>{summary.data.coverage_limitation && <small>{summary.data.coverage_limitation}</small>}</div>
        <div className="metrics-grid metrics-grid--four">
          <MetricCard icon="01" label="Source-supported jobs" value={formatNumber(sourceTechnologyJobs)} context={`${summary.data.snapshot.total_jobs} total observed jobs`} />
          <MetricCard icon="02" label="Source technology observations" value={formatNumber(sourceTechnologyObservations)} context={`${summary.data.snapshot.technology_observation_count} AI-backed observations`} />
      ...
      <Panel title="Technology observations" eyebrow="Source evidence and AI enrichment traceability" action={observations.data ? <span className="panel-note">{observations.data.total} observations</span> : undefined}>
        ...
        {observations.data && observations.data.items.length > 0 && <div className="table-wrap"><table><thead><tr><th>Technology</th><th>Support</th><th>Category</th><th>Related job</th><th>Business unit</th><th>Location</th><th>Evidence</th></tr></thead><tbody>{observations.data.items.map((item) => <tr key={`${item.job_id}-${item.normalized_technology}`}><td><strong>{item.normalized_technology}</strong>{item.technology !== item.normalized_technology && <span className="table-subtext">Source: {item.technology}</span>}</td><td>{titleCase(item.support_classification ?? "ai_enrichment")}</td><td>{item.category}</td><td><button className="job-title-button" onClick={() => setSelectedJob(item.job_id)}>{item.job_title}</button></td><td>{item.business_unit ?? <span className="muted">Not available</span>}</td><td>{item.location}</td><td><button className="job-title-button mono evidence-reference" title={item.evidence_id} onClick={() => setSelectedEvidence(item.evidence_id)}>{item.evidence_id.slice(0, 8)}…</button>{item.support_references[0]?.excerpt && <span className="table-subtext support-snippet">{item.support_references[0].excerpt}</span>}</td></tr>)}</tbody></table></div>}
      </Panel>
