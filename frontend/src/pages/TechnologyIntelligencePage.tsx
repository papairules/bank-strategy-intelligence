  ...
  return (
    <div className="page">
      ...
      <Panel title="Technology signals" eyebrow="Observed hiring intelligence" action={signals.data ? <span className="panel-note">{signals.data.generated_signal_count} signals</span> : undefined}>
        ...
        {signals.data?.signals.length === 0 && <div className="technology-signal-empty"><strong>Insufficient enrichment coverage for reliable technology signals.</strong><span>Technology observations exist, but {signals.data.enriched_jobs} of {signals.data.total_jobs} observed jobs are enriched ({formatPercent(signals.data.enrichment_coverage * 100, 1)} coverage). Broader signal generation is withheld.</span>{signals.data.limitations.length > 0 && <ul>{signals.data.limitations.map((item) => <li key={item}>{item}</li>)}</ul>}</div>}
        {signals.data && signals.data.signals.length > 0 && <div className="signals-list">{signals.data.signals.map((item) => <article className="signal-card" key={item.signal_id}><div className="signal-card__meta"><span>{titleCase(item.signal_type)}</span><span>{formatPercent(item.confidence * 100, 0)} confidence</span></div><h3>{item.title}</h3><p>{item.summary}</p><div className="signal-score"><span>Strength</span><div><i style={{ width: `${item.strength * 100}%` }} /></div><strong>{formatPercent(item.strength * 100, 0)}</strong></div><div className="technology-signal-metrics"><span>Evidence coverage <strong>{formatPercent(item.evidence_coverage * 100, 0)}</strong></span><span>{item.supporting_job_ids.length} jobs</span><span>{item.supporting_evidence_ids.length} evidence records</span></div><footer><span>{formatDate(item.observation_start)} – {formatDate(item.observation_end)}</span>{item.supporting_job_ids[0] && <button className="signal-reference-button" onClick={() => setSelectedJob(item.supporting_job_ids[0])}>View contributing job</button>}</footer>{item.limitations.length > 0 && <details><summary>Limitations</summary><ul>{item.limitations.map((limitation) => <li key={limitation}>{limitation}</li>)}</ul></details>}</article>)}</div>}
      </Panel>
